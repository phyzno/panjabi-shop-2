...
import { ChevronLeft, ChevronRight, Maximize2, X, PlayCircle, Info, Scissors, Image as ImageIcon } from 'lucide-react';
...
interface FabricQuickViewModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSelectFabric: (fabricId: string) => void; // কাস্টমাইজারে সিলেক্ট করার জন্য
}
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-[900px] rounded-[24px] bg-[#F7F7F2] shadow-[0_28px_80px_rgba(14,20,9,0.2)] overflow-hidden flex flex-col md:flex-row animate-in fade-in zoom-in-95 duration-300 max-h-[90vh]">
        ...
        {/* 🎯 PC ONLY: Vertical Divider Seam Button */}
        {fabric.video_url && (
          <button
        ...
        {/* 🛠️ FIX: overflow-visible এবং z-10 দেওয়ায় মোবাইলে বাটনটি সুন্দরভাবে ওভারল্যাপ করবে, কেটে যাবে না */}
        <div className={cn("w-full md:w-1/2 h-[40vh] min-h-[300px] md:h-auto md:aspect-[3/4] bg-[#EBECE3] shrink-0 relative overflow-visible md:overflow-hidden z-10 md:z-auto group/modalimg transition-all duration-300", isChangingVariant && "opacity-40 pointer-events-none")}>
          ...
          {showVideo && fabric.video_url ? (
            ...
            <>
              {allImages.length > 0 ? (
                <div onMouseMove={handleMouseMove} className="w-full h-full relative cursor-crosshair overflow-hidden rounded-t-[24px] md:rounded-none">
                  <img
          ...
          {/* 🎯 MOBILE ONLY: Sticky Floating Border Badge */}
          {fabric.video_url && (
            <button
              onClick={() => setShowVideo(!showVideo)}
        ...
        <div className={cn("w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-between overflow-y-auto", isChangingVariant && "opacity-40 pointer-events-none")}>
          <div>
            ...
            {fabric.description && (
              <div className="mb-6">
                {fabric.description.length > 250 ? (
                  !isExpanded ? (
                    /* 1️⃣ COLLAPSED STATE: Inline Read More Badge */
                    <p className="font-sans text-sm text-[#1C221A]/75 leading-relaxed tracking-wide">
                      {fabric.description.slice(0, 150).trim()}...
                      <button
                        type="button"
                    ...
                    /* 2️⃣ EXPANDED STATE: Smart Scrollable Descriptive Box */
                    <div className="animate-in fade-in slide-in-from-top-1 duration-200">
                      <div className="max-h-[140px] overflow-y-auto bg-[#EBECE3]/40 border border-[#D4D7C9]/60 rounded-xl p-3.5 font-sans text-sm text-[#1C221A]/80 leading-relaxed tracking-wide whitespace-pre-wrap pr-2 shadow-inner [scrollbar-width:thin] [scrollbar-color:#D4D7C9_transparent]">
                        {fabric.description}
                      </div>
                  ...
                  /* 3️⃣ SHORT TEXT STATE: Normal render if text is small */
                  <p className="font-sans text-sm text-[#1C221A]/75 leading-relaxed tracking-wide whitespace-pre-wrap">
                    {fabric.description}
                  </p>
                )}
              </div>
